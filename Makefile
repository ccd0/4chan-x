...
intermediate := LICENSE src/meta/fbegin.js tmp/declaration.js src/meta/helpers.js $(foreach p,$(parts),tmp/$(p).js) src/meta/fend.js
...
jshint := $(foreach f,$(subst API,API_crx API_userscript,$(parts)),.events/jshint.$(f))
...
tmp/declaration.js : $(wildcard src/*) tools/declare.js | tmp
	node tools/declare.js

tmp/helpers.js : src/meta/helpers.js | tmp
	$(CP)
...
tmp/$1.js : tmp/$1.coffee $(coffee_deps) tools/snip.js
	$(coffee) $$<
	node tools/snip.js $$@
...
tmp/API_%.js : tmp/API_%.coffee $(coffee_deps)
	$(coffee) $<
	node tools/snip.js $@ $(sources_API)
...
tmp/.jshintrc : src/meta/jshint.json tmp/declaration.js src/meta/helpers.js $(template_deps) | tmp
	$(template) $< $@
