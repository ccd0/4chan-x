QR.post = class
  constructor: (select) ->
    el = $.el 'a',
      ...
      draggable: true
      href: 'javascript:;'
    $.extend el, `<%= html('<a class="remove fourchan-x--icon" title="Remove">&{Icons.times_circle}</a><label class="qr-preview-spoiler"><input type="checkbox"> Spoiler</label><span></span>') %>`
