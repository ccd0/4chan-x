...
For a maintained fork of OneeChan (a style script used in addition to 4chan X), try
https://github.com/Nebukazar/OneeChan.

## Please note
**Uninstalling**: If 4chan X is uninstalled, then the native extension needs to be re-enable by clicking the `[Settings]` link in the top right corner, uncheck "`Disable the native extension`" in the panel, and click the "`Save Settings`" button. If the "`Save Settings`" button is not shown, then it may be being hidden by your ad blocker.

**Private browsing**: By default, 4chan X remembers the last read post in a thread and which posts were made by you, even when using private browsing / incognito mode. To turn this off, uncheck the `Remember Last Read Post` and `Remember Your Posts` options in the settings panel. You can clear all 4chan browsing history saved by 4chan X by resetting your settings.
...
**Chrome extension**: 4chan X is available as a standalone Chrome extension. The Chrome extension has the additional feature of being able to sync your settings and data with other devices via Chrome Sync. When the script updates an issue occurs: Whenever the Chrome extension is updated, until the tab is hard refreshed (F5), 4chan X is unable to save any data (such as posts marked as yours and settings changes). The userscript version above does not have this problem when 4chan X updates, only when Violentmonkey / Tampermonkey is updated. To install as a Chrome extension:
...
- **Midori**: Enable `User addons` in your preferences, under the Extensions tab. In the Privacy tab, check `Enable HTML5 local storage support`. Optionally, to be able to open new tabs when you start or reply to a thread, check `Allow scripts to open popups` under the Behavior tab. Then click the link to the [script](https://www.4chan-x.net/builds/4chan-X.user.js) to install it.

- **Luakit**: Navigate to the [script](https://www.4chan-x.net/builds/4chan-X.user.js), then type the command `:usi` to install it.

- **uzbl**: Install the script from https://github.com/singpolyma/singpolyma/blob/master/uzbl/data/scripts/userscript.sh, enable 4chan X in your config file, and then save [4chan X](https://www.4chan-x.net/builds/4chan-X.user.js) to `$XDG_DATA_HOME/uzbl/userscripts` (or `$HOME/.local/share/uzbl/userscripts`). The commands below assume you have ran uzbl at least once to create its config file.
...
## Beta version
New features and non-urgent bugfixes are released on the beta channel for further testing before they are moved the stable version. Please [report](https://github.com/ccd0/4chan-x/issues?q=is%3Aopen+sort%3Aupdated-desc) any issues and mention the version used. Settings should be back up regularly to prevent being lost due to bugs.

To install the **beta** version and get updates whenever there is a new **beta** version:
