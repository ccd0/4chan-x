...
### Chromium
**Userscript**: Install [Violentmonkey](https://chrome.google.com/webstore/detail/violent-monkey/jinjaccalgkegednnccohejagnlnfdag) or [Tampermonkey](https://tampermonkey.net/), then **[click here to install 4chan X](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js)**.
...
- **Chromium**, **Vivaldi**: **[Download 4chan X](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X.zip)**, then open `chrome://extensions`, enable Developer mode, and use "Load unpacked" to install the extracted zip. Alternatively, drag the zip file onto the page.
- **Opera**: **[Download 4chan X](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X.zip)**, then extract and load as an unpacked extension.
...
### Safari
Install the [Userscripts](https://itunes.apple.com/us/app/userscripts/id1463298887) extension. Enable it by pressing `⌘,`, navigating to the extensions pane and checking `Userscripts` checkbox. Now open the Userscripts editor by clicking on the `</>` button in the taskbar. Then click on the `+` button and select the `New Javascript` option. Replace the default text with the contents of the 4chan X **[script](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js)**. Finally save it by pressing `⌘s`.
...
- **dwb**: Install the userscripts extension, then save the [script](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js) to the `$XDG_CONFIG_HOME/dwb/greasemonkey` or `$HOME/.config/dwb/greasemonkey` directory (creating it if necessary):

  ```
  dwbem -N -i userscripts
  wget -P ${XDG_CONFIG_HOME:-$HOME/.config}/dwb/greasemonkey https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js
  ```

- **Midori**: Enable `User addons` in your preferences, under the Extensions tab. In the Privacy tab, check `Enable HTML5 local storage support`. Optionally, if you want 4chan X to be able to open new tabs when you start or reply to a thread, you will need to check `Allow scripts to open popups` under the Behavior tab. Then click the link to the [script](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js) to install it.

- **Luakit**: Navigate to the [script](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js), then type the command `:usi` to install it.

- **uzbl**: Install the script from https://github.com/singpolyma/singpolyma/blob/master/uzbl/data/scripts/userscript.sh, enable it in your config file, and then save [4chan X](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js) to `$XDG_DATA_HOME/uzbl/userscripts` (or `$HOME/.local/share/uzbl/userscripts`). The commands below assume you have run uzbl at least once to create its config file.
  ...
  wget -P "${XDG_DATA_HOME:-$HOME/.local/share}/uzbl/userscripts" https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js
  ```

- **qutebrowser**: Save the [script](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js) to the `$XDG_DATA_HOME/qutebrowser/greasemonkey` or `$HOME/.local/share/qutebrowser/greasemonkey` directory:

  ```
  wget -P ${XDG_DATA_HOME:-$HOME/.local/share}/qutebrowser/greasemonkey https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js
  ```

### MS Edge
Install [Tampermonkey](https://www.microsoft.com/en-us/store/p/tampermonkey/9nblggh5162s), then **[click here to install 4chan X](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js)**.
...
## Beta version
New features and non-urgent bugfixes are released on the beta channel for further testing before they are moved the stable version. Please [report](https://github.com/Anonyymi/4chan-x/issues?q=is%3Aopen+sort%3Aupdated-desc) any issues you find, and be sure to mention which version you're using. You should back up your settings regularly to prevent them from being lost due to bugs.

To install the **beta** version and get updates whenever there's a new **beta** version:
- [Install userscript](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js) (use with Greasemonkey / Violentmonkey / Tampermonkey / JS Blocker / etc.)
- [Download Chrome extension](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X.zip) (download, extract, and load as unpacked extension)
...
- [Install userscript](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X-noupdate.user.js)
- [Download Chrome extension](https://raw.githubusercontent.com/Anonyymi/4chan-x/support_miniboard/builds/4chan-X.zip)
